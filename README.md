# uKnow (ShadowTerm Edition) 🕶️🟢

uKnow は、サーバーを持たずに完全クライアントサイドで動作する、プライバシー重視の語学学習・シャドーイング支援アプリケーションです。すべての処理はブラウザ内で完結し、外部のバックエンドサーバーを必要としないため、サーバー維持費「0円」で高度なAI語学学習を実践できます。
...
本アプリをローカル環境や、自身のサーバーでビルド・ホスティングしたい方向けの起動手順です。uKnow は React + Vite で作られています。
